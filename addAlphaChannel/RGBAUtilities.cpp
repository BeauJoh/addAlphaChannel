...
uint8* convertToFourChannel (uint8* in){
    uint8* image = new uint8[getImageWidth()*getImageHeight()*4];
    ...
    for (int y =0; y < getImageHeight(); y++) {
        ...
        for (int x = 0; x< getImageWidth(); x++) {
            image[(y*getImageWidth()*4)+ newX +0] = in[(y*getImageWidth())+x+0];
            image[(y*getImageWidth()*4)+ newX +1] = in[(y*getImageWidth())+x+0];
            image[(y*getImageWidth()*4)+ newX +2] = in[(y*getImageWidth())+x+0];
            image[(y*getImageWidth()*4)+ newX +3] = 255;
            newX += 4;
        }    
    }
